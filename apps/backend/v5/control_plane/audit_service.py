...
class AuditService:
    ...
    def record_impersonation_start(
        self, admin_user_id: str, tenant_id: str, reason: str
    ...
    def record_impersonation_end(
        self, session_id: str
    ) -> ImpersonationSessionModel:
        session = (
            self._db.query(ImpersonationSessionModel)
            .filter(ImpersonationSessionModel.id == session_id)
            .first()
        )
        if session is None:
