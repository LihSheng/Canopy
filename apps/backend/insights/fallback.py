...
def build_fallback_summary(facts: FactBundle) -> str:
    lines: list[str] = []
    lines.append(f"HR Spend Executive Summary for {facts.current_month}\n")
    ...
    if facts.anomaly_count > 0:
        lines.append(
            f"\n{facts.anomaly_count} spending anomaly(s) were detected this period, requiring management attention."
        )
    else:
    ...
    if facts.top_departments:
        ...
        for d in facts.top_departments[:3]:
            lines.append(f"  - {d.name}: MYR {d.total_spend:,.2f} ({d.change_pct:+.1f}% vs previous month)")
...
def build_fallback_recommendations(facts: FactBundle) -> list[str]:
    ...
    if facts.anomaly_count > 0:
        recs.append("Review departments with detected anomalies for root cause analysis.")

    high_change = [d for d in facts.top_departments if abs(d.change_pct) >= 10.0]
    if high_change:
        names = ", ".join(d.name for d in high_change[:3])
        recs.append(f"Monitor month-over-month spend trends in departments with significant variance: {names}.")

    recs.append("Consider a quarterly spend review to identify cost optimisation opportunities.")

    if facts.top_departments:
        top = facts.top_departments[0]
        recs.append(f"Investigate claims composition in {top.name}, the highest-spend department this period.")

    recs.append("Set up automated anomaly thresholds to receive early warning for unusual spend patterns.")
...
def build_fallback_findings(facts: FactBundle) -> list[str]:
    ...
    if len(facts.department_rankings) >= 3:
        ...
        findings.append(
            "Top 3 departments by spend: " + ", ".join(f"{r.name} (MYR {r.total_spend:,.0f})" for r in top3) + "."
        )

    if facts.claim_type_breakdown:
        top_type = facts.claim_type_breakdown[0]
        findings.append(
            f"The largest claim category is '{top_type.type}' at MYR {top_type.amount:,.2f} ({top_type.count} claims)."
        )
