...
class TestDatasetReimportService:
    def test_reimport_valid_data_creates_new_active_version(self, tmp_path):
        ...
        try:
            ...
            dataset = Dataset(
                id=ds_id, project_id="p1", connection_id="c1", name="test", status=DatasetStatus.ACTIVE.value
            )
            dataset_repo.save(dataset)

            v1 = DatasetVersion(
                id=str(uuid.uuid4()), dataset_id=ds_id, version_number=1, status=DatasetVersionStatus.READY.value
            )
