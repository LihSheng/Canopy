...
from v5.queue.job_registry import JobRegistry, JobStatus
from v5.queue.tenant_queue import TenantJobQueue
...
class TestTenantJobQueue:
    def test_enqueue_adds_job_to_tenant_queue(self):
        queue = TenantJobQueue(max_concurrent_global=10)
        results = []

        def task(x):
            results.append(x)

        queue.enqueue("tenant-a", task, 42)
        item = queue.dequeue()
    ...
    def test_enqueue_preserves_tenant_isolation(self):
        ...
        queue.enqueue("tenant-a", task_a, 1)
        queue.enqueue("tenant-a", task_a, 2)
        queue.enqueue("tenant-b", task_b, 10)
        queue.enqueue("tenant-b", task_b, 20)
    ...
    def test_round_robin_fairness(self):
        queue = TenantJobQueue(max_concurrent_global=10)
        for i in range(5):
            queue.enqueue("tenant-a", lambda: None)
        for i in range(5):
            queue.enqueue("tenant-b", lambda: None)
        ...
        for item in items:
            jid = item[0]
            job = queue.registry.get_job(jid)
    ...
    def test_dequeue_returns_none_when_global_limit_reached(self):
        queue = TenantJobQueue(max_concurrent_global=1)
        queue.enqueue("tenant-a", lambda: None)
        first = queue.dequeue()
    ...
    def test_no_tenant_starvation(self):
        ...
        for item in items:
            job_id = item[0]
            job = queue.registry.get_job(job_id)
    ...
    def test_job_lifecycle_pending_to_completed(self):
        ...
        job_id = queue.enqueue("tenant-a", task)
        job_before = queue.registry.get_job(job_id)
        ...
        callable_fn(*args, **kwargs)
        queue.registry.update_status(jid, JobStatus.COMPLETED.value)

        job_after = queue.registry.get_job(job_id)
        assert len(completed) == 1
    ...
    def test_job_lifecycle_pending_to_failed(self):
        ...
        job_id = queue.enqueue("tenant-a", failing_task)
        item = queue.dequeue()
        assert item is not None
        jid, callable_fn, args, kwargs = item
        try:
            ...
            queue.registry.update_status(jid, JobStatus.FAILED.value, error="simulated failure")
    ...
    def test_registry_and_tracker_exposed(self):
        queue = TenantJobQueue()
        assert isinstance(queue.registry, JobRegistry)
        assert queue.tracker is queue._tracker

    def test_queue_is_thread_safe(self):
        queue = TenantJobQueue(max_concurrent_global=200)
        errors = []

        def enqueue_many(tid, count):
            try:
                for _ in range(count):
                    queue.enqueue(tid, lambda: None)
        ...
        for tid in [f"tenant-{i}" for i in range(4)]:
            total_jobs += len(queue.registry.get_tenant_jobs(tid))
        assert total_jobs == 200

    def test_dequeue_skips_tenant_at_concurrency_cap(self):
        queue = TenantJobQueue(max_concurrent_global=20)
        for _ in range(6):
            queue.enqueue("tenant-a", lambda: None)
        queue.enqueue("tenant-b", lambda: None)

        remaining_a = len(queue._queues.get("tenant-a", []))
        remaining_b = len(queue._queues.get("tenant-b", []))
