...
class TestFullBackupRestoreFlow:
    def test_full_backup_and_restore_flow(self, db_session, session_factory, backup_dir):
    ...
    def test_backup_creates_snapshot_ref(self, db_session, session_factory, backup_dir):
...
class TestDataIntegrityAfterRestore:
    def test_verify_tenant_data_after_restore(self, db_session, session_factory, backup_dir):
        _seed_tenant(db_session, "t-int-3")

        tenant_count_before = db_session.query(TenantModel).filter(TenantModel.lifecycle_state == "active").count()
        ...
        tenant_count_after = db_session.query(TenantModel).filter(TenantModel.lifecycle_state == "active").count()
        assert tenant_count_after == tenant_count_before


class TestRestoreLifecycleState:
    def test_restored_tenant_has_correct_lifecycle_state(self, db_session, session_factory, backup_dir):
...
class TestTenantScopedBackup:
    def test_backup_tenant_a_does_not_affect_tenant_b(self, db_session, session_factory, backup_dir):
        ...
        t_b_after = db_session.query(TenantModel).filter(TenantModel.id == "t-int-b").first()
        assert t_b_after is not None
        assert t_b_after.lifecycle_state == "active"


class TestArchiveAndRestoreFlow:
    def test_archive_then_restore_from_backup(self, db_session, session_factory, backup_dir):
        ...
        tenant = db_session.query(TenantModel).filter(TenantModel.id == "t-int-archive").first()
        tenant.lifecycle_state = "archived"
        db_session.commit()

        tenant_after_archive = db_session.query(TenantModel).filter(TenantModel.id == "t-int-archive").first()
