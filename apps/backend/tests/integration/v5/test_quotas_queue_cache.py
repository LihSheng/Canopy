...
class TestQueueFairnessIntegration:
    def test_two_tenants_five_jobs_each_round_robin(self):
        ...
        for i in range(5):
            queue.enqueue("tenant-a", make_task("tenant-a", i))
        for i in range(5):
            queue.enqueue("tenant-b", make_task("tenant-b", i))
    ...
    def test_concurrency_cap_per_tenant_prevent_excess(self):
        queue = TenantJobQueue(max_concurrent_global=30)
        for _ in range(10):
    ...
    def test_queue_with_real_threads(self):
        ...
        for i in range(3):
            queue.enqueue("tenant-a", make_task(f"a-{i}"))
        for i in range(3):
            queue.enqueue("tenant-b", make_task(f"b-{i}"))
...
class TestEndToEndQuotaQueueCache:
    ...
    def test_cache_and_queue_operate_independently(self):
        ...
        assert len(queue.registry.get_tenant_jobs("tenant-a")) == 1
